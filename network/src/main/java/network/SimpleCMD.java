...
import network.commands.Commands;
import network.parser.Parser;
import network.space.*;
...
/**
 * Class for initialisation map of network.commands.commands and running they.
 *
 * @version 0.4
 * @author Danhout.
 */
public abstract class SimpleCMD implements Commands {
    /**
     * The map for network.commands.commands.
     */
        ...
        try {
            ...
            if (method == null) {
                ...
                if (!command.equals("")) {
                    ...
                    err.println("help - a command that output the list with open network.commands.commands.");
                }
                return;
            }
     ...
     * The script contains network.commands.commands in the same format,
    ...
    public void executeScript(String[] args) throws IOException {
        ...
        try {
            ...
            // read network.commands.commands from the input's stream.
            do {
                // prompt to enter.
                out.print("$");
                out.flush();
     ...
     *
     * @return the name of the network.space marine.
     */
    protected String inputName() throws IOException {
        while (true) {
            // input from the stream line, check the network.space marine's name and return this.
            try {
    ...
    /**
     * Input a network.space marine.
     *
     * @return the network.space marine.
     */
     ...
     */
     protected String readConsoleLine() throws IOException {
        ...
        // if (script's execute) than: print the normal network.commands.commands to server's console.
        if (!stackReaders.isEmpty()) {
            out.println(str);
        }
        return str;
